 ...
 * "Care plan" tab of the client dashboard — it used to live at
 * /admin/care-plan/[patientId] behind its own sidebar tab. Staff assign
 * tasks from the catalog (plus custom entries); the RN supervisor approves
 * the list, and each approval is stamped per task. The ACTIVE + APPROVED
 * tasks render on the client's progress notes for per-shift charting, and
 * each task's TAR metadata (boxes per day, ordering physician, Appendix T
 * signed date) drives the monthly TAR grid.
 */
export default function CarePlanSection({ patientId }: { patientId: string }) {
  ...
  const reload = useCallback(async () => {
    if (!patientId) return;
    const t = await getCareTasks(patientId);
    setTasks(t);
  }, [patientId]);
  ...
  return (
    <div>
      <div style={sectionHeaderStyle}>
        <p style={subStyle}>
          Tasks on this list render on the client&apos;s progress notes once the RN supervisor
          approves them.
          {pending.length > 0 && (
...
// Site-standard select: custom chevron via withSelectChevron (a plain input
// style on a <select> keeps the mismatched native chevron).
const selectStyle: React.CSSProperties = withSelectChevron({ padding: '8px 10px', border: '1px solid #d0d7de', borderRadius: 8, fontSize: 13, fontFamily: 'inherit' });
