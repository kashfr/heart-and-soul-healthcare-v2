...
/**
 * Care Plans entry point (staff-only; the sidebar gates it to
 * admin/supervisor). Pick a client to build or review their plan-of-care
 * task list — the tasks their progress notes will chart against.
 */
export default function CarePlanPickerPage() {
    ...
    <div style={containerStyle}>
      <header style={headerStyle}>
        ...
        <div>
          ...
          <p style={subStyle}>Select a client to build or review their care task list.</p>
        </div>
      </header>
      ...
      {patients === null ? (
        ...
        <div style={emptyStyle}>
          {patients.length === 0 ? 'No clients found.' : 'No clients match your search.'}
        </div>
      ) : (
        <ul style={listStyle}>
          {filtered.map((p) => (
            <li key={p.id}>
              <Link href={`/admin/care-plan/${p.id}`} style={rowStyle}>
                <div style={{ minWidth: 0 }}>
                  <div style={nameStyle}>{p.name}</div>
                  <div style={metaStyle}>
                    {p.mrn ? `Record #${p.mrn}` : ''}
                    {p.hasFeedingTube ? `${p.mrn ? ' · ' : ''}Feeding tube` : ''}
                  </div>
                </div>
