  ...
  return (
    <div style={containerStyle}>
      <header style={headerStyle}>
        ...
        <div>
          <h1 style={titleStyle}>Treatments</h1>
          <p style={subStyle}>Select a client to open their treatment administration record.</p>
        </div>
      </header>
      ...
      {patients === null ? (
        ...
        <ul style={listStyle}>
          {filtered.map((p) => (
            <li key={p.id}>
              <Link href={`/admin/treatments/${p.id}`} style={rowStyle}>
                <div style={{ minWidth: 0 }}>
                  <div style={nameStyle}>{p.name}</div>
                  <div style={metaStyle}>{p.mrn ? `Record #${p.mrn}` : ''}</div>
                </div>
